...
export default function DungeonPage() {
  ...
  const { runs, addRun, removeRun, clearRuns } = useDungeonRuns();
  ...
  return (
    <div>
      <h1 className="text-3xl font-extrabold mb-6 text-yellow-400 drop-shadow">Dungeons</h1>
      <CharacterManager
      ...
      <div className="bg-gray-900 rounded-2xl shadow-xl border border-yellow-700 p-6">
        ...
        {runs.length === 0 ? (
          <div className="text-gray-500 py-4 text-center">No dungeon runs logged yet.</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="min-w-full text-sm rounded-xl overflow-hidden">
              ...
              <tbody>
                {runs.map((run) => (
                  <tr key={run.id} className="border-b border-gray-800 hover:bg-yellow-900/10 transition">
                    <td className="py-2 px-4 text-yellow-200">{getCharacterName(run.characterId)}</td>
                    <td className="py-2 px-4 text-yellow-100">{run.dungeon}</td>
                    <td className="py-2 px-4">
                      {(!run.loot || run.loot.length === 0)
                        ? <span className="bg-gray-800 text-gray-400 px-2 py-1 rounded-full border border-gray-700 text-xs">None</span>
