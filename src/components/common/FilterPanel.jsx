    ...
    <>
      ...
      {open && (
        <div
          ...
          onClick={() => setOpen(false)} // closes if you tap the backdrop
        >
          <div
            className="bg-gray-900 rounded-2xl shadow-2xl p-8 w-full max-w-lg relative"
            onClick={e => e.stopPropagation()} // don't close if you click inside the modal
          >
            <button
            ...
            {children}
            <button
              className="mt-8 w-full bg-yellow-400 text-gray-900 rounded-lg py-2 font-semibold hover:bg-yellow-300"
              onClick={() => setOpen(false)}
            >
              Apply Filters
            </button>
